...
public class TestLab1 {
    ...
    public void checkEfficiencyGoldenRatioMaximum(){
        Assert.assertEquals(MathToolkit.goldenRatioMaximum(x -> pow(x,3) - 2 * pow(x,2) - 5 * x + 6,
    ...
    public void checkEfficiencyFibonacci(){
        Assert.assertEquals(MathToolkit.fibonacci(x -> x * x - 4 * x + 4, 1, 3, 0.001),
                2.001195886151638, 1E-7);
        Assert.assertEquals(MathToolkit.fibonacci(x -> 1 / x - x, 1, 6, LOW),
                5.998172848529143, 1E-7);
        Assert.assertEquals(MathToolkit.fibonacci(x -> x * cos(x), 1, 10, MIDDLE),
